...
const WFHRequestForm = () => {
  ...
  const [form, setForm] = useState({ start_date: "", end_date: "", reason: "", type: "Full Day" })
  ...
  const loadRequests = async () => {
    setLoading(true)
    try {
      const data = await getWFHRequests()
      setRequests(data)
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true)
      await createWFHRequest(form)
      toast.success("WFH Request Submitted!")
      setForm({ start_date: "", end_date: "", reason: "", type: "Full Day" })
      loadRequests()
  ...
  const handleExportCSV = async () => { try { await exportWFHCSV(); toast.success("CSV exported!") } catch { toast.error("Failed to export CSV") } }
  const handleExportPDF = async () => { try { await exportWFHPDF(); toast.success("PDF exported!") } catch { toast.error("Failed to export PDF") } }
  ...
  const typeConfig = {
    "full day": { bg: "bg-blue-100 text-blue-700" },
    "half day": { bg: "bg-orange-100 text-orange-700" },
    "comp off": { bg: "bg-purple-100 text-purple-700" },
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Submit and track your WFH requests</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <input type="date" required
                  className="w-full border border-gray-300 px-3 py-2.5 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent"
              ...
              <div>
                ...
                <select className="w-full border border-gray-300 px-3 py-2.5 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white"
                  value={form.type} onChange={e => setForm({ ...form, type: e.target.value })}>
                  <option value="Full Day">Full Day WFH</option>
                  <option value="Half Day">Half Day WFH</option>
              ...
              <div className="md:col-span-2">
                ...
                <textarea required rows={4} placeholder="Describe the reason for your WFH request..."
                  className="w-full border border-gray-300 px-3 py-2.5 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent resize-none"
              ...
              <>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        {["Emp ID", "Employee", "Date Range", "Type", "Reason", "Status", "Rejection Reason", "Submitted"].map(h => (
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {currentRequests.map(r => {
                        ...
                        const tc = typeConfig[r.type?.toLowerCase()] || { bg: "bg-gray-100 text-gray-700" }
                        return (
                          <tr key={r.id} className="hover:bg-gray-50/70 transition-colors">
                            ...
                            <td className="px-5 py-3.5">
                              <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${tc.bg}`}>{r.type}</span>
                            </td>
                ...
                <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="px-3 py-1.5 text-xs font-semibold bg-green-600 text-white rounded-lg">{currentPage} / {totalPages}</span>
